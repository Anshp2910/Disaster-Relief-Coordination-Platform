...
export default function RequestForm({
  ...
  function getFormState(): FormState {
    return { title: formTitle, description, lat, lng, locationName, category, priority, peopleCount: String(peopleCount), status }
  }

  function applyFormState(state: Partial<FormState>) {
    setFormTitle(state.title || '')
    setDescription(state.description || '')
    setCategory(state.category || 'Other')
    setPriority(state.priority || 'Medium')
    setPeopleCount(Number(state.peopleCount) || 1)
      ...
      <div className="card">
        ...
        <form onSubmit={handleSubmit} className="inputGrid mt">
          ...
          <div>
            ...
            <input id="rf-people" type="number" min="1" max="10000" value={peopleCount} onChange={(e) => { setPeopleCount(Number(e.target.value)); markDirty() }} className="w-full" />
          </div>
