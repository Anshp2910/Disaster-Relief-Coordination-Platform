  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', maxHeight: 500 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', borderBottom: '1px solid var(--gov-border)' }}>
        ...
        {onClose && <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 18 }}>&times;</button>}
      </div>
        ...
        {messages.map((m) => {
          ...
          return (
            <div key={m._id || m.createdAt} style={{ display: 'flex', justifyContent: isSystem ? 'center' : isMe ? 'flex-end' : 'flex-start' }}>
              {isSystem ? (
                <div style={{ fontSize: 11, color: '#999', background: 'rgba(0,0,0,.04)', padding: '3px 10px', borderRadius: 12 }}>{m.text}</div>
              ) : (
                <div style={{ maxWidth: '75%' }}>
                  {!isMe && <div style={{ fontSize: 11, color: 'var(--gov-blue)', marginBottom: 2, fontWeight: 600 }}>{m.sender?.displayName || 'User'}</div>}
                  <div style={{ background: isMe ? 'var(--gov-blue)' : 'rgba(0,0,0,.06)', color: isMe ? 'white' : 'inherit', padding: '7px 12px', borderRadius: isMe ? '12px 12px 2px 12px' : '12px 12px 12px 2px', fontSize: 13 }}>
                    {m.text}
                  </div>
                  <div style={{ fontSize: 10, color: '#999', marginTop: 2, textAlign: isMe ? 'right' : 'left' }}>{formatTime(m.createdAt)}</div>
                </div>
              )}
            </div>
          )
      ...
      <form onSubmit={handleSend} style={{ display: 'flex', gap: 8, padding: '10px 12px', borderTop: '1px solid var(--gov-border)' }}>
        ...
        />
        <button type="submit" className="btnPrimary" disabled={!text.trim()} style={{ fontSize: 12, padding: '6px 16px' }}>Send</button>
      </form>
    </div>
  )
}
