...
export default function Incidents() {
  ...
  useEffect(() => {
    ...
    incidents.forEach((inc) => {
      ...
      const marker = L.marker([inc.centerLat, inc.centerLng], { icon }).addTo(map)
      ...
      marker.bindPopup(`
        <div style="font-family:Arial,sans-serif;min-width:180px">
          ...
          <div style="font-size:12px;margin-bottom:6px"><span style="color:${color};font-weight:600">${inc.severity}</span> &middot; ${inc.status}</div>
  ...
  function openCreate() {
    ...
    setForm({ name: '', description: '', disasterType: 'Other', severity: 'Medium', status: 'Active', affectedPopulation: '', centerLat: '20.5937', centerLng: '78.9629' })
    setShowForm(true)
  }
  ...
  function openEdit(inc) {
    setEditIncident(inc)
    setForm({
      name: inc.name, description: inc.description || '', disasterType: inc.disasterType, severity: inc.severity,
      status: inc.status, affectedPopulation: String(inc.affectedPopulation || ''), centerLat: String(inc.centerLat || ''), centerLng: String(inc.centerLng || ''),
    })
    setShowForm(true)
  }
    ...
    <div className="container">
      <div className="card">
        <div className="headerRow">
          ...
          {currentUser?.role === 'admin' && <button className="btnPrimary" onClick={openCreate}>Create Incident</button>}
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); setPage(1); load() }} style={{ display: 'flex', gap: 8, marginTop: 12 }}>
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search incidents..." style={{ flex: 1, padding: '8px 12px', border: '1px solid var(--gov-border)', borderRadius: 6, fontSize: 13 }} />
          <button type="submit" className="btnPrimary" style={{ fontSize: 12, padding: '6px 16px' }}>Search</button>
        </form>

        <div style={{ display: 'flex', gap: 6, marginTop: 12, flexWrap: 'wrap' }}>
          {['All', 'Active', 'Monitoring', 'Resolved', 'Closed'].map((s) => (
            <button key={s} onClick={() => { setFilterStatus(s); setPage(1) }} className={`filter-pill ${filterStatus === s ? 'active' : ''}`}>{s}</button>
          ))}
        </div>
        <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
          {['All', 'Flood', 'Earthquake', 'Cyclone', 'Drought', 'Fire', 'Landslide', 'Other'].map((d) => (
            <button key={d} onClick={() => { setFilterDisaster(d); setPage(1) }} className={`filter-pill ${filterDisaster === d ? 'active' : ''}`} style={{ fontSize: 11 }}>
        ...
        {selectedIncident && (
          <div className="card" style={{ width: 320, flexShrink: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              <h3 style={{ margin: 0, fontSize: 15, color: 'var(--gov-blue)' }}>{DISASTER_ICONS[selectedIncident.disasterType]} {selectedIncident.name}</h3>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
              <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: SEVERITY_COLORS[selectedIncident.severity] + '20', color: SEVERITY_COLORS[selectedIncident.severity], border: `1px solid ${SEVERITY_COLORS[selectedIncident.severity]}40` }}>{selectedIncident.severity}</span>
              <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: 'rgba(0,0,128,.1)', color: '#000080', border: '1px solid rgba(0,0,128,.3)' }}>{selectedIncident.status}</span>
            </div>
            {selectedIncident.description && <div style={{ fontSize: 13, marginBottom: 8, lineHeight: 1.5 }}>{selectedIncident.description}</div>}
            <div style={{ fontSize: 13, lineHeight: 1.8 }}>
              <div>Type: <strong>{selectedIncident.disasterType}</strong></div>
              {selectedIncident.affectedPopulation > 0 && <div>Affected: <strong>{selectedIncident.affectedPopulation.toLocaleString()}</strong></div>}
      ...
      {showForm && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="card" style={{ width: 500, maxHeight: '90vh', overflow: 'auto' }}>
            <h3 style={{ margin: '0 0 12px', fontSize: 16, color: 'var(--gov-blue)' }}>{editIncident ? 'Edit Incident' : 'Create Incident'}</h3>
            <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 8 }}>
              <input placeholder="Incident name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
