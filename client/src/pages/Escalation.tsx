...
import { AlertTriangle, ArrowUp, ArrowDown, Search } from 'lucide-react'
      ...
      <div className="container">
        <motion.div variants={containerVariants} initial="hidden" animate="visible">
          ...
          <div className="flex-between mb-md mt-md">
            <div>
              <h1 className="page-title">{t('nav.escalation') || 'Request Escalation Pipeline'}</h1>
              <p className="text-sm text-muted mt-xs">
                {t('escalation.subtitle')} &middot; {items.length} {t('escalation.escalated')}
              </p>
            </div>
            <div className="flex gap-sm items-center">
              <button onClick={() => setShowForm(true)} className="btn-primary btn-sm">
                <ArrowUp size={14} />
                <span>{t('escalation.escalateRequest')}</span>
              </button>
            </div>
          </div>
          ...
          <div className="search-input-wrapper" style={{ flex: '1 1 200px', maxWidth: 320, marginBottom: 'var(--space-sm)' }}>
            ...
            <input
              ...
              aria-label={t('escalation.searchPlaceholder') || 'Search escalations'}
            />
          </div>
          ...
          {loading ? (
            ...
            <EmptyState
              ...
              title={search ? t('escalation.noMatch') : t('escalation.noEscalated')}
              description={t('escalation.noEscalatedDesc') || 'No escalated requests at this time.'}
            />
          ) : (
            <div className="card">
              <div className="flex-between mb-sm">
                <h2 className="page-title-sm flex items-center gap-xs">
                  ...
                  {t('escalation.escalated') || 'Escalated Requests'}
                </h2>
              </div>
              <div className="grid-gap">
                {filteredItems.map((item) => (
                  <div key={item._id} className="list-card border-l-4 border-l-danger">
                    <div className="flex-between gap-sm">
                      <div className="flex-1 min-w-0">
                        <div className="text-bold text-accent">{item.title}</div>
                        <div className="text-sm text-danger mt-xs flex items-center gap-xs">
                          <AlertTriangle size={14} />
                          {new Date(item.escalatedAt).toLocaleString()}
                        </div>
                        <div className="mt-xs text-sm">{item.escalationReason}</div>
                        <div className="text-xs text-muted mt-xs">
                          {t('escalation.by')} {item.createdBy?.displayName || t('common.unknown')}
                        </div>
                      </div>
                      <button
                        onClick={() => handleDeescalate(item._id)}
                        className="btn-danger btn-sm flex items-center gap-xs flex-shrink-0"
          ...
          {/* Escalate Modal */}
          <Modal open={showForm} onClose={() => { setShowForm(false); setError('') }} title={t('escalation.escalateRequest')}>
            <form onSubmit={handleEscalate}>
                  ...
                  {showReqDropdown && (
                    <div className="ms-dropdown mt-xs">
                      {filteredRequests.length === 0 ? (
                        ...
                        <div className="overflow-y-auto p-xs" style={{ maxHeight: 200 }}>
              ...
              <div className="flex gap-sm">
                ...
                <button type="button" onClick={() => { setShowForm(false); setError('') }} className="btn-ghost btn-sm">{t('common.cancel')}</button>
              </div>
            </form>
          </Modal>
