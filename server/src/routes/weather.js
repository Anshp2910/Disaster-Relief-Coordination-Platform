...
router.get('/current', validateQuery(weatherQuery), async (req, res) => {
  try {
    ...
    if (!resp.ok) {
      logger.error('[weather] upstream error', { status: resp.status })
      return res.status(502).json({ success: false, error: 'Weather service unavailable', code: 'UPSTREAM_ERROR' })
    }
    ...
    return sendServerError(res, 'Failed to fetch weather data')
  }
})
